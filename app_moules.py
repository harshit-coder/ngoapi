...
from database import db_connect, logger, options
...
def return_states():
    try:
        ...
        for p in range(0, len(c)):
            if c['extract_status'][p] == "row insert success" or c['extract_status'][p] == "row update success":
                if c['state_name'][p] not in d1:
                    d1[c['state_name'][p]] = {'total_ngos': c['total_ngos'][p], 'pages': {}}
                if c['page_no'][p] not in d1[c['state_name'][p]]['pages']:
                    d1[c['state_name'][p]]['pages'][c['page_no'][p]] = []
                d1[c['state_name'][p]]['pages'][str(c['page_no'][p])].insert(p, {'ngo_row': str(c['row_no'][p]), 'ngo_name': str(c['ngo_name'][p]), 'ngo_address': str(c['ngo_address'][p]), 'ngo_mobile': str(c['ngo_mobile'][p]), 'ngo_email': str(c['ngo_email'][p]), 'reg_date': str(c['ngo_reg_date'][p])})

            if c['state_name'][p] not in d2:
                d2[c['state_name'][p]] = {'total_pages': c['pages'][p], 'total_ngos': c['total_ngos'][p]}
            if c['extract_status'][p] == "row insert success":
                if c['extract_status'][p] not in d2[c['state_name'][p]]:
                    sql = 'SELECT COUNT(row_no) FROM ngo_details WHERE state_name ="{}" AND extract_status ="{}"'.format(c['state_name'][p], c['extract_status'][p])
                    q = connect.query_database_df(sql)
                    d2[c['state_name'][p]][c['extract_status'][p]] = {'no_of_rows': str(q['COUNT(row_no)'][0])}
                if c['url'][p] not in d2[c['state_name'][p]][c['extract_status'][p]]:
                    d2[c['state_name'][p]][c['extract_status'][p]][c['url'][p]] = []
                d2[c['state_name'][p]][c['extract_status'][p]][c['url'][p]].append("Row no: " + str(c['row_no'][p]))

            if c['extract_status'][p] == "row insert fail":
                if c['extract_status'][p] not in d2[c['state_name'][p]]:
                    sql = 'SELECT COUNT(row_no) FROM ngo_details WHERE state_name ="{}" AND extract_status ="{}"'.format(c['state_name'][p], c['extract_status'][p])
                    q = connect.query_database_df(sql)
                    d2[c['state_name'][p]][c['extract_status'][p]] = {'no_of_rows': str(q['COUNT(row_no)'][0])}
                if c['url'][p] not in d2[c['state_name'][p]][c['extract_status'][p]]:
                    d2[c['state_name'][p]][c['extract_status'][p]][c['url'][p]] = []
                d2[c['state_name'][p]][c['extract_status'][p]][c['url'][p]].append("Row no: " + str(c['row_no'][p]))
            if c['extract_status'][p] == "row update success":
                if c['extract_status'][p] not in d2[c['state_name'][p]]:
                    sql = 'SELECT COUNT(row_no) FROM ngo_details WHERE state_name ="{}" AND extract_status ="{}"'.format(c['state_name'][p], c['extract_status'][p])
                    q = connect.query_database_df(sql)
                    d2[c['state_name'][p]][c['extract_status'][p]] = {'no_of_rows': str(q['COUNT(row_no)'][0])}
                if c['url'][p] not in d2[c['state_name'][p]][c['extract_status'][p]]:
                    d2[c['state_name'][p]][c['extract_status'][p]][c['url'][p]] = []
                d2[c['state_name'][p]][c['extract_status'][p]][c['url'][p]].append("Row no: " + str(c['row_no'][p]))
            if c['extract_status'][p] == "row update fail":
                if c['extract_status'][p] not in d2[c['state_name'][p]]:
                    sql = 'SELECT COUNT(row_no) FROM ngo_details WHERE state_name ="{}" AND extract_status ="{}"'.format(c['state_name'][p], c['extract_status'][p])
                    q = connect.query_database_df(sql)
                    d2[c['state_name'][p]][c['extract_status'][p]] = {'no_of_rows': str(q['COUNT(row_no)'][0])}
            ...
            if c['extract_status'][p] == "page insert success":
                if c['extract_status'][p] not in d2[c['state_name'][p]]:
                    d2[c['state_name'][p]][c['extract_status'][p]] = {}
                    sql = 'SELECT COUNT(page_no) FROM ngo_details WHERE state_name ="{}" AND extract_status ="{}"'.format(c['state_name'][p], c['extract_status'][p])
                    q = connect.query_database_df(sql)
                    d2[c['state_name'][p]][c['extract_status'][p]] = {'no_of_pages': str(q['COUNT(page_no)'][0])}
                if c['page_no'][p] not in d2[c['state_name'][p]][c['extract_status'][p]]:
                    d2[c['state_name'][p]][c['extract_status'][p]]["Page no: "+ str(c['page_no'][p])] = "Url: " + str(c['url'][p])

            if c['extract_status'][p] == "page insert fail":
                if c['extract_status'][p] not in d2[c['state_name'][p]]:
                    d2[c['state_name'][p]][c['extract_status'][p]] = {}
                    sql = 'SELECT COUNT(page_no) FROM ngo_details WHERE state_name ="{}" AND extract_status ="{}"'.format(c['state_name'][p], c['extract_status'][p])
                    q = connect.query_database_df(sql)
            ...
            if c['extract_status'][p] == "page update success":
                if c['extract_status'][p] not in d2[c['state_name'][p]]:
                    d2[c['state_name'][p]][c['extract_status'][p]] = {}
                    sql = 'SELECT COUNT(page_no) FROM ngo_details WHERE state_name ="{}" AND extract_status ="{}"'.format(c['state_name'][p], c['extract_status'][p])
                    q = connect.query_database_df(sql)
            ...
            if c['extract_status'][p] == "page update fail":
                if c['extract_status'][p] not in d2[c['state_name'][p]]:
                    d2[c['state_name'][p]][c['extract_status'][p]] = {}
                    sql = 'SELECT COUNT(page_no) FROM ngo_details WHERE state_name ="{}" AND extract_status ="{}"'.format(c['state_name'][p], c['extract_status'][p])
                    q = connect.query_database_df(sql)
        ...
        exc_type, exc_obj, exc_tb = sys.exc_info()
        logger.info("exception in return_state " + str(e) + ' ' + str(exc_tb.tb_lineno))
